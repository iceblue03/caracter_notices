...
- **캐릭터 상세** — 캐릭터 소개, 관련 해시태그, 그 캐릭터의 굿즈 매물, 그 캐릭터의 모든 소식.
...
```
server.ts                 Express: 로컬 개발/자체 호스팅용 (이미지 프록시 · feed/goods sync · 번역)
...
data/                     커밋되는 스크래핑 캐시 (feed-store.json · goods-store.json)
src/
  characters.ts           구독 가능한 캐릭터 카탈로그 (키워드·해시태그·소스 계정)
  data.ts                 번들 샘플 소식 · 샘플 굿즈
  events.ts               수집된 게시물에서 추린 실제 행사 (홈 피드 배너용)
  types.ts                Character · FeedPost · GoodsListing · PostMatch 타입
  ...
  components/             Sidebar · FeedView · GoodsView · EventBanner · DiscoverView ·
                           CharacterDetailView · PostCard · GoodsCard …
```
